...
class GameOfLife {
  ...
  generateGolExample(): [Tensor2D, Tensor2D] {
    const world =
        tf.randomUniform([this.size - 2, this.size - 2], 0, 2, 'int32') as
        Tensor2D;
  ...
  private countNeighbors(size: number, worldPadded: tf.Tensor2D): tf.Tensor2D {
    ...
    return neighborCount as Tensor2D;
  }

  /* Helper method to pad an array until the op is ready. */
  private static padArray(array: Tensor2D): Tensor2D {
    ...
    return tf.tensor2d(values, shape as [number, number]);
  }
}
